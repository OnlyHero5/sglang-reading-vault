...
- multi_agent：仍用 `custom_generate`，但一个输入样本会产出多个 agent sample。
- rollout_buffer：用独立 HTTP 服务做外部轨迹队列，再通过 `rollout_function` 拉回训练。
- GLM5 插件：不是 runnable example，而是模型结构级插件，展示 `slime_plugins/` 可以深入 Megatron model provider。
...
源码依据：`examples/README.md` L3-L20 把 examples 定位为可复用 RL workflow；`slime_plugins/rollout_buffer/README.md` L3-L18 把 rollout buffer 定位为独立异步 agent trajectory 生成组件。
...
| 一个 prompt 并行多个 agent | multi_agent | `--custom-generate-function-path` |
| 轨迹生成跑在外部集群 | rollout_buffer | `--rollout-function-path` + HTTP buffer |
| 新模型结构或 Bridge | GLM5 / megatron_bridge | import side effect 或 model provider |
