...
| pending task | `asyncio.Task[group]` | `remaining_batch_size` 按 group 计数 |
...
```python
# 来源：slime/rollout/sglang_rollout.py L174-L203
...
- fan-out list：缺 reward 的 sibling 走 `batched_async_rm`。
- group RM：等整个 group 完成后统一 `batched_async_rm`。
...
这就是为什么 [[Slime-数据源]] 里强调默认 data source 必须支持 `add_samples`，否则 partial rollout 没法回收。
...
这些测试都偏契约，不会启动真实 SGLang server。真实服务排障还要结合 router 日志、`sglang_generate` trace、`perf/` metrics 和 `rollout/` metrics。
