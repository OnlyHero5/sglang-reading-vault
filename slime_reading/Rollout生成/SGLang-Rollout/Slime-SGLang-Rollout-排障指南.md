...
```python
# 来源：slime/utils/arguments.py L328-L339
            parser.add_argument(
...
- 确认 `over_sampling_batch_size >= rollout_batch_size`，默认参数会补齐，但自定义 args 可能破坏。
...
| custom generate 优先级与 fan-out | `python -m pytest slime/tests/plugin_contracts/test_plugin_generate_contracts.py -q` |
| rollout function 签名与返回包装 | `python -m pytest slime/tests/plugin_contracts/test_plugin_rollout_contracts.py -q` |
| DataSource partial 回灌 | `python -m pytest slime/tests/plugin_contracts/test_plugin_path_loading_contracts.py -q` |

这些测试在当前机器可能受缺失依赖影响。如果 import 阶段报 `httpx`、`ray`、`sglang_router` 或 Torch/NumPy 兼容问题，先记录环境问题，不要把它归因于 SGLang Rollout 逻辑。
