...
`discover_external_engines` 的推导规则很窄：GPU 数优先读 `num_gpus` 或 `num_gpus_per_engine`，否则用 `tp_size * pp_size`。
...
测试把这个模型固定下来：`tp_size=4`、`pp_size=2` 时，如果 server 没显式给 `num_gpus`，Slime 推导出 8 张 GPU。

```python
# 来源：slime/tests/test_external_sglang_engines.py L30-L58
