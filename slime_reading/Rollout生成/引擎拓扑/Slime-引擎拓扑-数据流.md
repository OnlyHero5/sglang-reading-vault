...
```python
# 来源：slime/ray/placement_group.py L102-L132
...
本地多模型部署会给每个模型创建 Router，但默认 `generate` 只使用 `args.sglang_router_ip/port`，也就是第一个模型的 Router。多模型选路是 custom rollout 的责任。
...
```python
# 来源：slime/ray/rollout.py L1117-L1126
...
```python
# 来源：slime/ray/rollout.py L382-L410
def offload(self):
    handles = []
