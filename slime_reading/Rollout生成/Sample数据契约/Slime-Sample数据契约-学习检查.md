...
- [ ] 能说明 top-p replay 的 `token_ids + offsets` ragged 表结构，以及 offsets 长度为什么是 `response_length + 1`。
...
这类测试需要 stub 环境能正常导入 SGLang/Ray 相关模块；如果当前环境缺 `ray`、`sglang_router`、`httpx` 等依赖，可能在 import 阶段失败。
