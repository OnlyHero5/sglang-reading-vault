...
```python
# 来源：slime/utils/types.py L114-L144
response: str = ""
...
```python
# 来源：slime/rollout/sglang_rollout.py L278-L286
else:
...
| `all_data` | 所有完成的 groups，包括 drop 的 groups |

```python
# 来源：slime/rollout/sglang_rollout.py L426-L439
...
```python
# 来源：slime/rollout/sglang_rollout.py L561-L569
...
```python
# 来源：slime/rollout/sglang_rollout.py L608-L614
...
| `--rollout-all-samples-process-path` | 已收集 `all_data` 后 | 可处理包含 drop 的全部完成 groups |

参数说明中明确 `rollout_sample_filter` 不决定 advantage normalization：

```python
# 来源：slime/utils/arguments.py L1416-L1425
parser.add_argument(
