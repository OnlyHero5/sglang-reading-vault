...
node maintenance/audit_source_evidence.mjs --note slime_reading/Rollout生成/数据源/Slime-数据源-数据流.md
node maintenance/audit_source_evidence.mjs --note slime_reading/Rollout生成/数据源/Slime-数据源-排障指南.md
```

预期现象：引用的 upstream 文件存在，行号范围有效。
