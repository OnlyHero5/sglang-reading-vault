...
| external engine 不符合预期 | 外部 server 的 server args 与 Slime 期望不一致 | `discover_external_engines`、`_init_external` | 请求 `/server_info` 或 `/get_server_info` |
...
因为 Slime 把多节点 engine 的 HTTP 控制面收敛到 node 0。`ServerGroup.engines` 只返回每个 engine 的 node 0 actor，`_make_request` 在非 node 0 直接返回。
...
```python
# 来源：slime/backends/sglang_utils/sglang_engine.py L234-L245
...
`flush_cache` 非 200 会重试，60 次后抛 `TimeoutError`。
...
Slime 不启动和不杀外部 SGLang 进程，但仍会发现 server、推导拓扑、创建 Ray adapter、校验 server args、注册 router。
...
- server args 不一致，`_init_external` 会 assertion fail。
...
```python
# 来源：slime/ray/rollout.py L996-L998
if worker_type == "prefill":
    addr_and_ports[current_rank]["disaggregation_bootstrap_port"] = get_port()
```

```python
# 来源：slime/backends/sglang_utils/sglang_engine.py L216-L230
payload = {
    "url": worker_url,
...
```python
# 来源：slime/backends/megatron_utils/update_weight/update_weight_from_distributed.py L288-L304
world_size = sum(engine_gpu_counts) + 1  # +1 for training rank 0
...
1. `len(rollout_engines)` 是否等于 node 0 engine 数。
...
```python
# 来源：slime/backends/megatron_utils/actor.py L613-L624
...
验证方法：打开 CI 检查时，训练侧会随机挑 engine 比对版本。
...
| disk delta | 传输少 | 每 host 本地 checkpoint 应用必须覆盖 | 带宽受限或远端 FS 慢 |
