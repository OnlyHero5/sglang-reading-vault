...
| 全节点集合 | `RolloutServer.all_engines` | 所有 actor list | delta disk 每个 host 应用本地 checkpoint |
...
```python
# 来源：slime/ray/rollout.py L296-L330
@property
def engines(self):
...
| `nccl_port` | 同上 | SGLang 内部 distributed | 同一个 engine 内 TP/PP 通信 |
...
## 权重更新数据流：tensor 与 disk

| 路径 | 控制面 payload | 数据落点 | 适合场景 |
|------|----------------|----------|----------|
| tensor | `serialized_named_tensors`、`load_format`、`weight_version` | GPU IPC 或远端 distributed fallback | colocate 同机 |
| disk | `model_path`、`load_format`、`weight_version`、`files` | checkpoint 目录或 delta 文件 | 跨节点、超大模型、文件系统可靠 |
...
- Slime 推导的 `worker_type`、GPU 数和 bootstrap port 要和实际 server 匹配。
- `shutdown`、`recover`、`offload` 的语义和本地模式不同，不能用本地模式的日志预期套 external。
...
5. external 模式保留 Slime 控制面，移走的是本地进程生命周期。
