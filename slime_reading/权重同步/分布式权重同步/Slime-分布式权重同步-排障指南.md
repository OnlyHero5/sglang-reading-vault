...
## 2. 为什么只有 DP=0、TP=0 的 rank 发权重

PP stage 内，TP ranks 共同持有同一层的分片；DP ranks 是副本。只有 DP=0、TP=0 的 rank 代表该 PP stage 对 engine 发 metadata 和 NCCL broadcast。
...
## 10. Direct iterator 要不要画进 NCCL 主线
...
## 12. `update_weights_interval` 会改变什么
