...
flowchart LR
    ...
    Bucket["bucket<br/>buffer-size bounded"]
...
bucket 按 `update_weight_buffer_size` 切分。专家参数用 expert TP size 估算，其余参数用普通 TP size 估算，因为 full param 会在 gather 后展开。

```python
# 来源：slime/backends/megatron_utils/update_weight/hf_weight_iterator_direct.py L108-L136
...
读者抓手：导出 OOM 时，不一定是 converter 错；先看 bucket size 和 full param 重建峰值。
...
```python
# 来源：slime/backends/megatron_utils/update_weight/hf_weight_iterator_direct.py L24-L41
...
## 7. disk sync 的版本目录是同一条数据流的消费者

`UpdateWeightFromDisk` 在版本目录中调用同一个 saver，完成后通知 rollout engine 从磁盘 reload。它没有自己的 converter。

```python
# 来源：slime/backends/megatron_utils/update_weight/update_weight_from_disk.py L65-L96
