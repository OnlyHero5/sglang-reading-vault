...
设计选择：先构造 `rollout_id_to_samples`，保留 rollout id 首次出现顺序；每 `global_batch_size` 个 rollout 构成一个 step；尾部不满 step 的 rollout 被丢弃。
...
```python
# 来源：slime/utils/dp_schedule.py L167-L189
...
```python
# 来源：slime/utils/dp_schedule.py L191-L209
...
```python
# 来源：slime/backends/megatron_utils/data.py L55-L67
...
差异：默认 CP 先对每条样本切片再 concat；allgather-CP 先 concat 全局 token stream，再按 CP rank chunk。
...
```python
# 来源：slime/backends/megatron_utils/data.py L120-L148
loss_masks = []
...
```python
# 来源：slime/backends/megatron_utils/model.py L576-L600
batch = get_batch(
    data_iterator,
