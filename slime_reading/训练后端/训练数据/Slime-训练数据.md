...
Train Data 是一个 **批次整形器**：先按 rollout id 切训练 step，再把 step 内样本 pack 成 micro-batch，分给 DP ranks；Actor 侧按 schedule 播放样本，`get_batch` 把 list-of-samples 整形成 Megatron 的 packed THD 输入。
