...
- [ ] 能说出 `start_rollout_id = loaded_rollout_id + 1` 的来源，并解释为什么所有 rank 返回值必须一致。
...
rg -n ($terms -join '|') slime_reading/训练后端/Megatron-Actor初始化
```
...
| 配置 | 预期现象 |
|------|----------|
| 正常训练 | 所有 rank 返回同一个 `start_rollout_id`，driver 不触发一致性 assert |
