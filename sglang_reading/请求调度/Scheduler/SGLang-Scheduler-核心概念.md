...
stateDiagram-v2
    ...
    Prefill --> Running: prefill result 合并<br/>进入 running_batch
...
这张状态图比“函数列表”更重要。Scheduler 每轮循环都在回答：有没有新请求应该先 prefill？已有 running 请求能不能继续 decode？GPU forward 的上一轮结果是否已经处理？KV pool 是否够下一步用？
...
这里的“prefill 优先”不是说 decode 不重要，而是 serving 的用户体验通常先被 TTFT 打穿；已有 decode 请求已经在 `running_batch` 中保留状态，每轮可以继续推进。真正的平衡由 `PrefillAdder`、token budget、batch full、prefill delayer 和 KV 可用量共同决定。
...
## PP 是另一套事件循环，不是默认 overlap 的小变体

当 `pp_size > 1`，事件循环分派到 `event_loop_pp()`，而不是 `event_loop_overlap()`。PP 的核心压力是 stage 间 bubble 和 proxy tensor 通信，不是单进程内 CPU/GPU 两拍 overlap。
