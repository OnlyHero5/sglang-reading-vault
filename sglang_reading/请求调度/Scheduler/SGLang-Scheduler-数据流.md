...
flowchart LR
    ...
    PROC --> OUT["OutputStreamer<br/>send_to_tokenizer"]
```
...
- `mm_inputs` 通过 pickle wrapper 延迟展开，避免跨进程传输时过早处理。
...
```python
# 来源：python/sglang/srt/managers/scheduler.py L2912-L2957
...
```python
# 来源：python/sglang/srt/managers/scheduler.py L3204-L3286
...
- overlap 下 `batch.copy()` 被放入 `result_queue`，防止当前 batch 对象被下一轮调度改写。
...
## 对象 7：PP stage 传的是请求和 proxy，不是普通 result queue

PP loop 每个 stage 维护 microbatch 槽位。非末 stage 发送请求和 proxy tensor，末 stage 处理 output。它的主要边界是 stage 间通信，而不是 default overlap 的 `result_queue`。
...
PP 的可跳过输出优化也很保守，只允许纯 chunked prefill 中间 chunk 且无 logprob 时跳过 output comm。
