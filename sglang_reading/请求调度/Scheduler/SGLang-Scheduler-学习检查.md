...
- [ ] 能画出 `waiting_queue`、`last_batch`、`running_batch`、`cur_batch` 的迁移关系。
...
- [ ] 能区分 normal loop、overlap loop、PP loop 的状态推进方式。
...
flowchart LR
    ...
    Extend --> Last["last_batch"]
    Last --> Running["running_batch"]
...
- 能说明 `last_batch` 为什么存在：prefill forward 完成后，还要在下一轮并入 `running_batch`。
...
> 一个 tokenized generate 请求进入 Scheduler 后，为什么它先变成 `Req`，为什么可能进入不同队列，什么时候从 `waiting_queue` 变成 EXTEND batch，EXTEND 结束后如何进入 `running_batch`，decode 前如何检查 KV，overlap 下为什么输出处理可能晚一拍，最后结果如何回到 TokenizerManager？
