...
| 调度记录 | `Req` | 已命中的 KV indices | `prefix_indices`、`last_node`、`cache_protected_len` |
...
`prefix_indices` 是已经在 device 上可用的 KV indices；`host_hit_length` 是 HiCache 相关分叉；`last_node` 是 lock 和后续 unfinished/finished cache 的锚点。
...
这个状态迁移有两个容易误读的点：`cache_protected_len` 表示 tree 接管到哪里；`prefix_indices` 可能还附带未进 tree 的 tail，供后续 chunk 继续跳过。
...
所以 Unified 的数据流不是“另一套 prefix cache”，而是“同一前缀 key 下，有多种资源状态需要同步维护”。
