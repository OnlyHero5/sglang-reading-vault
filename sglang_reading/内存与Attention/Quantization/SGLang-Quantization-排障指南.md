...
| GPTQ 普通 method 遇到 MoE 报错 | 绑定账 | `GPTQConfig.get_quant_method` |
| AWQ MoE 没走 Marlin | 绑定账 | `AWQConfig.get_quant_method` |
...
`get_quantization_config` 会先检查字符串是否在 `QUANTIZATION_METHODS` 中。这个错误说明还没有进入 HF config 解析，更没有进入 layer 或 kernel。
...
## 3. 显式 FP8 backend 不可用：不要把它当 auto fallback

显式 `--fp8-gemm-backend=flashinfer_trtllm` 会检查 SM100/SM103 和 FlashInfer 可用性。不满足直接报错。
...
- 如果你显式指定 backend，就按“强约束”理解：不支持应该失败，而不是悄悄换 kernel。
- benchmark 前开启 kernel 日志，确认实际执行路径。

## 4. static/dynamic FP8 scale 搞混：看 activation 形态

`scaled_fp8_quant` 用 `scale is None` 区分 dynamic 和 static。dynamic 又分 per-token 和 per-tensor；static 要求传入标量 scale。
...
- 输出精度异常但没有 load 错时，优先确认 activation scale 是 static 还是 dynamic。
...
## 6. 普通 GPTQ 遇到 MoE 报错：换 MoE 支持路径

普通 `GPTQConfig` 对 `FusedMoE` 直接报错，提示使用 `gptq_marlin`。
...
## 7. AWQ MoE 没走 Marlin：可能是安全 fallback

AWQ 对 Linear 和 MoE 分别做 Marlin 支持检查。MoE 不满足时回退到 Moe WNA16，并打一条 warning。
...
- 如果 scale 保持初始无效值，说明 checkpoint 没加载到 scale 或 postprocess 没执行。

## 9. unquant MoE 为什么还有 quant info

无量化 MoE 仍走统一 runner。runner 需要一个描述权重 dtype、EP/TP rank 和 routed scaling 的 quant info，对 bf16 也一样。
...
- 对 MoE 来说，quant info 是 runner ABI 的一部分；bf16 也要用同一套调用结构。
...
3. layer 类型是否拿到了正确 method。
4. `create_weights` 是否通过 shape 与 TP 对齐检查。
5. `model.load_weights` 后是否执行 postprocess。
