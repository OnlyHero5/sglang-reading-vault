...
| 理解插件何时生效 | 第 4、7、8、9 节 | 插件必须在 runtime import 和 engine 启动前加载，并通过 hook registry patch 目标对象 |
...
flowchart TD
 ...
 D --> E["load_plugins"]
 D --> F["_extract_model_type_override"]
 F --> G["get_model_path"]
...
## 4. `serve()` 在 runtime import 前加载插件
...
自动检测按 overlay、本地目录、known registry、远程 `model_index.json` 顺序判断。任何异常都返回 False，让调用方 fall through 到 LLM 路径。
...
这不是强分类器，而是保守确认器。想绕过自动检测，要显式传 `--model-type llm` 或 `--model-type diffusion`。
...
`load_plugins` 幂等执行通用插件，然后统一调用 `HookRegistry.apply_hooks()`。
...
如果 hook 在 runtime import 之后才 apply，某些模块可能已经持有旧引用。启动链路把插件阶段放在 `prepare_server_args` 和 `run_server` 之前，就是为了降低这种风险。
...
读插件问题时要看 hook 类型：BEFORE 改输入，AFTER 改输出，AROUND 控制原函数调用，REPLACE 替换目标。
...
如果 argv 含 `--config`，`prepare_server_args` 会先构造 `ConfigArgumentMerger`，把 YAML 转成 CLI 参数，再交给 argparse。
...
```python
# 来源：python/sglang/srt/server_args.py L6862-L6870
...
构造 `ServerArgs` 会触发 `__post_init__`。这就是很多启动错误发生在 parse 之后、进入 HTTP 之前的原因。

## 13. `run_server` 根据 `ServerArgs` 分叉

`run_server` 的判断顺序就是 runtime 分叉优先级。
...
- 插件是启动前控制面，不是请求时扩展点。
- `run_server` 是最后分叉，默认 HTTP 的内部细节属于下一专题。
