...
| `ServerArgs` | dataclass 配置事实 | `from_cli_args` + `__post_init__` | `run_server`、HTTP/gRPC/Ray/Encoder |
...
sequenceDiagram
 ...
 LP->>HR: apply_hooks()
 HR-->>Runtime: runtime objects patched before serving
```
...
`plugin_hook` 本身只注册 hook，真正修改目标在 `apply_hooks()`。
...
| 参数组合 ValueError | post-init | `ServerArgs.__post_init__` |
| Ray ImportError | runtime branch | `run_server` |
| ZMQ IPC 地址异常 | runtime derived | `PortArgs.init_new` |

## 复盘

启动链路的数据流是单进程控制面，不是服务请求数据面。它的核心对象从字符串逐步变成结构化事实，再进入 runtime 分支；插件则作为并行控制面，在 runtime import 前修改目标对象。
