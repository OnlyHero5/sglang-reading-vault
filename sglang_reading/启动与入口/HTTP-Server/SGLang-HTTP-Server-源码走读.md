...
| 判断“端口已开”和“服务可用” | 第 11、12 节、运行验证 | `/health` 可以是轻量探活，warmup 会发真实请求，把模型、detokenizer 和 scheduler 一起压通 |
...
`lifespan` 先区分 single/multi tokenizer，再初始化 metrics、tracing 和协议 handler。之后启动 warmup thread，并在应用退出时关闭 tool server、join warmup thread。
...
```python
# 来源：python/sglang/srt/entrypoints/http_server.py L378-L391
...
深度探活的成功条件不是 task 完成，而是在 timeout 内 tokenizer manager 收到后端响应并更新 `last_receive_tstamp`。
...
这解释了为什么 readiness probe 要谨慎选择：轻量 `/health` 低成本，但不证明 Scheduler 与 Detokenizer 可用；`/health_generate` 更强，但会穿过后端链路。

## 12. warmup 把“监听中”推进到“可服务”

warmup 先轮询 `/model_info`，失败则杀进程树，避免留下端口可达但内部不可用的服务。
...
warmup 不是简单性能预热，它是一次端到端可用性推进：handler、TokenizerManager、Scheduler、Detokenizer 至少要能完成一次最小请求。
...
预期：默认轻量 health 在服务非 Starting 且未 graceful exit 时返回 `200`。如果要验证后端链路，用 `/health_generate`，它会走一次最小后端往返。
