...
| 症状 | 先查源码入口 | 常见原因 | 验证 |
|------|--------------|----------|------|
| `/health` 返回 503 | `health_generate` | graceful exit、`ServerStatus.Starting`、深度探活超时 | 查 health 日志和 `server_status` |
| 服务端口开了但不 ready | `_wait_and_warmup`、`_execute_server_warmup` | warmup 未完成或失败 | 查 `/model_info` 和 warmup error |
| `--tokenizer-worker-num > 1` 加 API key 启动失败 | `init_multi_tokenizer` | multi-tokenizer 模式不支持 API key | 搜 assert 文本 |
| OpenAI route 报 handler 缺失 | `lifespan` | app 被 import 但 lifespan 未完成 | 查 `app.state.openai_serving_*` |
...
curl -s -o NUL -w "%{http_code}" http://127.0.0.1:30000/health_generate
```

预期：前者只证明服务非 Starting 且未 graceful exit；后者还要穿过 tokenizer manager 和后端响应路径。

## 4. warmup 失败为什么会杀进程树？

warmup 先等 `/model_info`，如果 120 次轮询内都失败，源码直接 `kill_process_tree(os.getpid())`。这不是过度激进，而是在避免“端口可达、内部不可用”的半初始化实例继续接流量。
...
3. 如果只是开发调试，可显式跳过 warmup；生产环境应修复模型加载、handler、CUDA graph 或后端链路问题。
...
HTTP Server 排障要先分类：启动链、worker 初始化、route 委托、readiness、下游推理。不要看到文件名是 `http_server.py` 就把所有问题都归为 FastAPI 问题。
