...
| 出站 | 内部累积 chunk | OpenAI SSE / Ollama NDJSON | delta 重复、reasoning/tool call 状态错位 |
...
如果 Chat 输出格式错，不要先看 `TokenizerManager`；先看这一步到底给模型喂了什么 prompt 和约束。
...
所以“为什么 stream 请求有时返回 JSON error 而不是 SSE error”不是偶然，而是这个 kick-start 设计。
...
这里是排查 stream 形状的第一现场：`stream_offsets` 控制普通文本，`reasoning_parser_dict` 控制思考内容，`parser_dict` 控制工具调用。
...
Ollama stream 同样把内部累积 text 切 delta，但外层协议是 NDJSON。
...
这说明“delta 计算”是共同问题，“SSE vs NDJSON”只是外层包装差异。
