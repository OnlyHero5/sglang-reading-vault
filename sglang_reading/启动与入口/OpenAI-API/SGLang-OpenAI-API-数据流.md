...
`raw_request` 后面会被用于 custom labels、routing key、DP rank header 和 abort background task。
...
StreamingResponse 的 background task 会在连接断开后尝试 abort 请求。这个逻辑在 TokenizerManager，而不是 Chat/Completion handler。
...
Embedding 也走 `OpenAIServingBase`，但内部请求是 `EmbeddingReqInput`，不走 Chat 的 stream 状态机。它先校验 input 非空、类型一致、token id 非负。
...
Ollama 是另一套协议入口，但仍然复用 TokenizerManager。它的 options 会先转成 SGLang sampling params，并给 `max_new_tokens` 一个更长的默认值。
...
排查 Ollama 输出长度时，先看 `num_predict` 是否映射到 `max_new_tokens`，以及默认值是否符合预期。
