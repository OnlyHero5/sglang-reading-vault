...
## Q7：客户端断开后 Scheduler 会继续生成吗？

设计上不会。Rust stream drop 会触发 `RequestAbortGuard`，它调用 `bridge.abort`，Python 再调用 `TokenizerManager.abort_request`。
...
如果断连后 KV 或 running request 不释放，要沿 `rid` 查三处：Rust stream 是否 drop、`bridge.abort` 是否执行、`TokenizerManager.abort_request` 是否收到。
...
原因是 Python 内部没有单独的 gRPC classify runtime 通道，分类输出复用 embedding 请求对象和生成入口。
...
生产判断：内网或 sidecar/gateway 前置认证是当前更合理的暴露方式。
