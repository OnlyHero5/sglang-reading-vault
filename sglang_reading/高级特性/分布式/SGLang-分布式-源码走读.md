...
这篇按“WORLD 如何被切成多套模型并行 group”读：`init_distributed_environment` 先接住 PyTorch WORLD，`initialize_model_parallel` 做 world size、TP、PP、Attention、MoE、DCP 的硬校验和分组，所有 group 再被包装成 `GroupCoordinator`，模型层通过 `communication_op.py` / getter 进入对应 collective。
...
源码不会先问“这是第几张卡”，而是先建立 `WORLD=[0,1,2,3,4,5,6,7]`，然后一层层投影：

```mermaid
flowchart TB
  W["WORLD ranks 0..7"] --> TP["TP groups<br/>[0,1,2,3] [4,5,6,7]"]
  W --> PP["PP groups<br/>[0,2,4,6] [1,3,5,7]"]
...
`initialize_model_parallel` 的第一道门槛很硬：`world_size` 必须等于 `tensor_model_parallel_size * pipeline_model_parallel_size`。注意这里没有乘 `dp_size`，因为 DP Controller 会启动多个 Scheduler worker，每个 worker 内部再按 TP/PP 建模型并行。
...
TP 先切出来，是因为 Attention、MoE、DCP 都在 TP 内部继续分解。后续很多“为什么这个 group 只覆盖部分 rank”的问题，都要回到当前 TP group 的局部坐标里解释。
...
这里还有一个细节：Attention TP 明确关闭 custom all-reduce 和 MSCCL++ all-reduce，只有在同步 token id 或 symmetric memory 时启用 PyNccl。这是 backend 策略，不是 layer 代码自己决定。
...
MoE EP group 负责 expert parallel 的坐标。源码给 `_MOE_EP` 关闭 PyNccl 与 custom all-reduce，因为 MoE token dispatch 的路径和普通 TP all-reduce 不同。
...
PP group 使用 `range(pp_group_idx, world_size, num_pipeline_model_parallel_groups)`。在 `world_size=8, pp_size=2` 时，会得到 `[0,2,4,6]` 和 `[1,3,5,7]`。
...
2. CPU tensor 走 shared memory allreduce 或 torch distributed。
...
| 目标 | 方法 | 预期现象 |
|------|------|----------|
| 验证坐标合法性 | 用 `tp_size * pp_size` 对照进程数 | 不相等时在 `initialize_model_parallel` 前置校验报错 |
| 验证 TP group | 在 `get_tp_group` 或 `tensor_model_parallel_all_reduce` 打断点 | layer 侧不会直接进入裸 `torch.distributed.all_reduce` |
| 验证 backend 选路 | 在 `GroupCoordinator.all_reduce` 观察 `outplace_all_reduce_method` | 小 tensor、graph、symmetric memory、CustomAllReduce 会走不同分支 |
