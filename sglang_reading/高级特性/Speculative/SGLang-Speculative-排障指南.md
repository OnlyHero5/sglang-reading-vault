...
| 症状 | 先查 | 源码入口 | 验证方法 |
|------|------|----------|----------|
| 打开投机后吞吐下降 | accept rate 和 draft/verify 成本 | `eagle_sample`、`AdaptiveController.on_verify_complete` | 对比关闭投机、降低 steps/topk 后 tokens/s |
...
| target verify 输出跨 TP 不一致 | 验收账 | `eagle_sample` broadcast | 看 `predict/accept_index/num_correct_drafts` 是否 rank 0 broadcast |
...
## Q4：为什么 verify 后 TP rank 必须同步采样结果？

target logits 经过 softmax、top-k、top-p 后，TP rank 间可能因浮点微差采到不同 token。spec decode 的后果比普通 sampling 更重，因为 `accept_index` 还会驱动 KV 写回。
...
排障抓手：如果多卡输出偶发 diverge，不能只比较 `predict`，还要比较 `accept_index` 和 `num_correct_drafts`。这三个必须同源。
...
DFLASH 能在 draft 阶段支持 target verify for draft；普通 EAGLE 的 target verify 是独立阶段。源码用算法能力谓词表达这个差异。
...
因此遇到 “draft 阶段为什么触发 target verify” 这类问题，应先确认算法是不是 DFLASH，而不是把它套入普通 EAGLE 的两阶段模型。

## Q8：adaptive spec 为什么没切换，或频繁切换？

adaptive controller 要先为候选 steps 构建 runtime state，然后在 batch size 或 verify 结果触发时 `_activate`。如果 state 未注册，切换会失败；如果 accept 反馈抖动，可能频繁切换。
...
3. 接着看验收结果：`predict`、`accept_lens`、`accept_index` 是否一致。
4. 最后看 KV 和 runtime state：accepted KV 是否移动，adaptive 是否切换。
