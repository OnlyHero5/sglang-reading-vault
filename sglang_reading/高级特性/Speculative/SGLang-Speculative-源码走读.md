...
这篇沿一轮 decode step 走源码：启动参数先决定 worker，worker 再提出候选，target verify 产生 logits，采样得到接受路径，最后把 accepted token 的 KV 写回 target 主链。读完应能把一次投机 decode 从控制面追到 KV 写回，而不是只知道 EAGLE 或 NGRAM 的名字。

## 长文读法

这篇按“候选怎么来、target 怎么验、验收结果怎么写回”读：`SpeculativeAlgorithm` 先把参数规范化并选择 worker，`BaseSpecWorker` 规定 target / draft / adaptive 的外壳契约，EAGLE family 用 draft worker 生成候选，NGRAM 用语料匹配生成候选，但两者最终都要进入 target verify、`eagle_sample` 和 `move_accept_tokens_to_target_kvcache` 这一条验收账。
...
| 排查 target verify 逻辑 | 5 到 6 | target 只负责正式 logits，接受长度、接受索引和最终 token 由采样函数收口 |
| 理解 rejection sampling 或 TP 不一致 | 6 | greedy / rejection sampling 在同一函数内分支，采样结果需要跨 TP rank 同步 |
| 排查 accepted token 没进主 KV | 7 | topk 大于 1 时要先 compact 接受路径，再把临时 verify KV 搬到 target 主链 |
| 对比 NGRAM 与 EAGLE | 8 | NGRAM 没有独立 draft model，但仍复用 verify、采样和 KV 写回这套验收账 |
| 看 adaptive 为什么会变步数 | 9 | verify 后的接受统计会反馈给 runtime state，下一轮按 batch size 和 EMA 切配置 |

读的时候把“草稿生成”和“正式验收”分开：草稿侧可以换算法，正式验收侧必须统一地产出 `predict`、`accept_lens`、`accept_index`，否则后面的 KV 写回和调度结果都无法保持一致。
...
flowchart TD
  ...
  C --> D[EAGLEWorkerV2 或 NGRAMWorker]
  D --> E[forward_batch_generation]
  E --> F[draft model 或 NGRAM corpus]
  F --> G[VerifyInput 写入 batch.spec_info]
  G --> H[target_worker.forward_batch_generation is_verify]
  H --> I[eagle_sample]
  I --> J[predict accept_lens accept_index]
  J --> K[move_accept_tokens_to_target_kvcache]
  K --> L[GenerationBatchResult]
```

这条主线有两个分叉：EAGLE family 有独立 draft worker，NGRAM 没有 draft model；topk 大于 1 时，EAGLE 需要把接受路径 compact 到每个请求块前部。
...
- DFLASH、Frozen-KV MTP、EAGLE family、STANDALONE、NGRAM 都收敛到 V2 worker schema。
...
## 6. 采样：greedy、rejection sampling 和 TP 同步在同一个函数收口

系统压力：target logits 只说明 target 认为每个候选位置的分布如何，还没说明哪条路径被接受。`eagle_sample` 把 target 分布、draft 分布、采样约束和 TP 一致性收敛成三个输出。
...
这里的 broadcast 是排障关键点：TP rank 上 softmax、top-k、top-p 的浮点微差可能导致采样结果不同，因此最终 `predict`、`accept_index`、`num_correct_drafts` 以 rank 0 为准。
...
`eagle_sample` 返回的 `accept_lens` 包含 bonus token；KV mover 需要的是正确 draft 数，所以 EAGLE topk path 传 `accept_lens - 1`。topk 大于 1 时，还要把 accepted path compact 到每个请求块前部。
...
## 8. NGRAM：没有 draft worker，但 verify 和写回仍复用同一套验收账
...
NGRAM 的核心不变量是：match state 绑定请求 rid，不是绑定 batch slot。请求离开 decode batch 后若不 erase，下一批请求可能继承旧上下文的匹配状态。

## 9. Adaptive：验收结果反过来切 runtime state

adaptive spec 不在采样前猜测收益，而是在 verify 后消费接受长度。controller 预先注册候选 steps 的 runtime state，并在 batch size 或 EMA 触发时切换。
...
这解释了为什么“accept rate 低”不是单独 metric 问题：它可能导致下一轮 draft steps、draft token 数、CUDA graph batch set 和 attention backend state 一起切换。
...
| 打开 NGRAM | 无 draft worker，仍进入 target verify 和 KV mover |
| topk 大于 1 | verify 后会执行 accepted path compact |
| 开启 adaptive | verify 后的 accepted drafts 会喂给 controller，后续 step 可能切换 |

调试时建议在这些入口打断点：`SpeculativeAlgorithm.create_worker`、`EAGLEWorkerV2.verify`、`eagle_sample`、`move_accept_tokens_to_target_kvcache`、`NGRAMWorker.forward_batch_generation`、`AdaptiveController.on_verify_complete`。
