...
SamplingParams
  ...
  -> masked / penalized logits
  -> batch_next_token_ids
  -> Req grammar / penalty state
```
...
`SamplingParams` 是单个请求的采样意图。它包含 API 字段和内部字段，但只有 normalize 后的内部字段才是后续稳定消费对象。
...
`SamplingBatchInfo` 还会记录 batch 级开关：是否全 greedy，是否需要 top-p、top-k、min-p。
...
模型输出的 `next_token_logits` 会被原地改写。`apply_logits_bias` 的名字容易误导，它实际上还包括 penalty、grammar mask 和 logit bias。
...
这两个 backend 的 mask 形态不同，但对 Sampler 的效果相同：非法 token 在后续采样中不可选。
...
Sampler 输出的是 `[bs]` 的 token id。约束解码下，SGLang 会同步 token id，避免 TP rank 因采样差异进入不同 grammar 状态。
...
这是约束解码下 TP hang 的关键边界：token id 一旦不同，后续 grammar FSM 也会不同。

## 7. 状态推进：grammar 接受 token

正常 decode 和 speculative verify 都需要推进 grammar FSM。spec decode 会在 grammar 结束时截断 overdrafted suffix，避免非法后缀进入 KV 或输出。
...
| Logits 预处理 | `[bs, vocab]` logits | masked/penalized logits | mask、penalty、bias 是否存在 |
| Sampler | logits/probs | `batch_next_token_ids` | greedy vs backend 分支 |
| 结果处理 | token ids | updated `Req` state | grammar accept、penalty cumulate |

## 运行验证

这篇可以先不启动模型，用源码检索验证三条主线是否仍然成立：参数规范化在 `SamplingParams`，batch 侧状态在 `SamplingBatchInfo`，采样后状态推进在 `BatchResultProcessor`。

```powershell
rg -n 'class SamplingParams|def normalize|class GrammarManager|class SamplingBatchInfo|def update_regex_vocab_mask|def apply_logits_bias|BatchedPenalizerOrchestrator|apply_scaling_penalties|def _sync_token_ids_across_tp|def _accept_grammar_tokens' sglang/python/sglang/srt/sampling/sampling_params.py sglang/python/sglang/srt/constrained/grammar_manager.py sglang/python/sglang/srt/sampling/sampling_batch_info.py sglang/python/sglang/srt/sampling/penaltylib/orchestrator.py sglang/python/sglang/srt/layers/sampler.py sglang/python/sglang/srt/managers/scheduler_components/batch_result_processor.py
```

读输出时重点看三件事：

- `normalize()` 仍然负责把 API alias、stop 字段和内部标志收束到 `SamplingParams`。
- `SamplingBatchInfo` 仍然同时承载 grammar mask、penalty orchestrator 和 logits bias。
