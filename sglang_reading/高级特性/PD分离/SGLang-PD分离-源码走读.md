...
这篇解决一个具体问题：一条 generate 请求被拆给 Prefill 节点和 Decode 节点后，源码如何保证 Decode 不会在 KV、metadata 或 HiCache restore 未就绪时提前进入 running batch。
...
这篇按“同一条请求在 Prefill 和 Decode 两边分别卡在哪个 gate”来读。不要把 PD 分离理解成 Prefill 算完后直接把 KV 交给 Decode；源码实际拆成房间号、receiver 预分配、sender bootstrap、metadata 写入、KV chunk 发送、poll/all-reduce、commit、`PREBUILT` batch 八个边界。

| 读者任务 | 先读 | 要抓住的判断 |
|----------|------|--------------|
| 第一次建立 PD 主线 | 主线图、步骤 1 到 3 | Decode 会先创建 receiver 并占好接收位置，Prefill 不是唯一的起点 |
| 请求一开始就被拒绝 | 步骤 1、步骤 2 | 真实 backend 下 `bootstrap_room` 不能缺失，缺失会作为 bad request abort |
| Prefill 端没有发出 KV | 步骤 4、步骤 5 | sender 先 bootstrap，再在最后 chunk 写 metadata 并发送 KV page/state payload |
...
```mermaid
flowchart LR
  A["请求入口<br/>GenerateReqInput"] --> B["分词后对象<br/>TokenizedGenerateReqInput"]
  B --> C["Scheduler<br/>Req"]
  C --> D["DecodePreallocQueue<br/>创建 KVReceiver"]
  D --> E["receiver send_metadata<br/>预分配 KV 与 metadata slot"]
  C --> F["PrefillBootstrapQueue<br/>创建 KVSender"]
  F --> G["Prefill extend forward<br/>产生首 token 和 KV"]
...
这条链路最容易读错的地方是：Prefill 和 Decode 都先收到同一条请求，但两边做的事不同。Decode 先占接收位置，Prefill 再把 KV 和首 token metadata 送过去。只有 receiver poll、metadata gate、跨 rank 共识、可选 HiCache restore 都通过后，Decode 才把请求变成 `PREBUILT` batch。

## 步骤 1：请求先带上房间号

系统压力：PD 把一个请求拆到两个进程甚至两组 GPU 上，必须有一个跨进程主键。SGLang 选择把这个主键放在请求结构里，而不是让 transfer backend 私下猜。
...
## 步骤 3：Decode 先创建 receiver 并预分配

系统压力：Prefill 计算完 prompt 后要有地方发 KV。Decode 不能等真正 running slot 空出来才握手，否则 Prefill 会堵在 bootstrap。
...
这里的心理模型是“Decode 先把收货地址、货架位置和收货单号发出去”。Prefill 后面发送的 KV page 和 metadata，都要落到这些预先约定的位置。
...
设计选择很明确：`create_sender` 只建立连接对象，`finalize_bootstrap` 才分配 metadata slot、读取 Decode 已有 prefix、初始化要发送的 page 数。这样 bootstrap 未完成或 metadata slot 暂时耗尽时，请求不会错误进入 waiting forward。
...
2. Decode 卡 transfer：先看 `KVPoll`、metadata room、staging 和 HiCache gate。
3. Decode 进入 waiting 但不 running：看 `max_running_requests`、`req_to_token_pool.size`、grammar waiting 和 running batch 空位。
...
- Decode 先预分配，Prefill 后发送，这个顺序决定了 bootstrap 卡顿时应该先查 Decode。
