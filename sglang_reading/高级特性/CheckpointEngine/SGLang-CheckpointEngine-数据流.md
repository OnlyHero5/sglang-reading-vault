...
sequenceDiagram
  ...
  S->>S: flush cache / barrier / metrics
  S-->>TM: UpdateWeightsFromIPCReqOutput
  TM-->>HTTP: success/message
  HTTP->>HTTP: initial_weights_loaded=True
```

四条状态线分别是：

- 启动状态线：`initial_weights_loaded=False -> True -> warmup -> Up`。
- 控制面状态线：HTTP request -> TokenizerManager writer lock -> communicator result。
...
这条线只负责“初始权重是否已通过 update 成功加载”。不要把它和 `/ping` 混为一谈。
...
这条线的核心不变量是 `inference_parallel_size`。它决定 torchrun rank 分组、src rank、socket path 切片和 SGLang TP 的对齐关系。
...
这条线回答并发问题：IPC update 不是随意和请求并发读写模型，它要经过 TokenizerManager 的更新控制。
...
这里有两个状态边界：主 TP 成功决定 cache flush；最终 success 还可能受 draft worker 影响。
...
热更新成功后要 flush cache，因为 KV cache 与旧权重绑定。源码只在请求 `flush_cache=True` 时执行 flush。
...
所以热更新面板应把 `weight_load_duration_seconds{source="ipc"}`、`num_paused_reqs` 和 `cache_hit_rate` 分开看。
...
| `/ping` 成功但 warmup 没继续 | 启动状态线 | `_wait_weights_ready` | 查 `initial_weights_loaded` 是否被成功 update 置 True |
...
| 热更新后 cache hit 降 | cache 线 | `flush_cache_after_weight_update` | 默认 flush 是预期行为 |
| duration metric 不动 | metrics 线 | `_observe_weight_load` | 确认 metrics collector 存在且 update 进入 WeightUpdater |
| draft model 失败 | Scheduler 执行线 | `WeightUpdater.update_weights_from_ipc` | 看最终 success/message 和 draft worker log |

## 复盘

- CheckpointEngine 有三类 ready：HTTP 可达、初始权重 ready、server warmup 完成。
- HTTP 控制面只传 handles；权重数据线走 checkpoint-engine worker 和 ZMQ。
- TokenizerManager 负责并发控制；Scheduler WeightUpdater 负责 serving 安全边界。
- GPU UUID、`inference_parallel_size`、TP rank 布局必须一致。
- cache flush 和 metrics 是热更新语义的一部分，不是附加日志。
