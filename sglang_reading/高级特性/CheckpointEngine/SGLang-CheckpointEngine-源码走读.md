...
这篇沿一次真实 IPC 热更新走源码：SGLang 用 dummy 权重启动并等待；外部 `update.py` 通过 `/ping` 确认 HTTP 可达；ParameterServer 暴露每张 GPU 的 ZMQ handle；HTTP `/update_weights_from_ipc` 通知 SGLang；Scheduler 暂停、加载、flush cache、barrier；成功后释放 `initial_weights_loaded`。

读完后要能判断：一个失败发生在外部脚本、HTTP 控制面、TokenizerManager 锁、Scheduler 执行面、ModelRunner 适配层，还是第三方 checkpoint-engine worker。

## 长文读法

这篇按“server 先 ready 到 HTTP，但推理 ready 要等 IPC 权重灌入”读：SGLang 启动时把 `initial_weights_loaded` 置为等待状态，外部 `update.py` 只等 `/ping` 可达，再通过 ParameterServer 和 `/update_weights_from_ipc` 把 ZMQ handles 交给执行面，成功后才释放初始权重 ready。

| 读者任务 | 先读 | 要抓住的判断 |
|----------|------|--------------|
| 第一次建立 IPC 热更新主线 | 贯穿场景、1 到 3 | HTTP listen、warmup ready、初始权重 ready 是三个不同状态 |
...
flowchart TB
  ...
  I --> L["flush cache + barrier"]
  G --> M["initial_weights_loaded=True"]
```
...
## 第 2 步：TokenizerManager 维护初始权重 ready 状态
...
`_wait_and_warmup` 在 warmup 前检查等待开关。开启后，`_wait_weights_ready` 每秒轮询 `initial_weights_loaded`，超时只记录 error。
...
这一步解释一个常见误解：外部脚本能访问 `/ping`，不代表 warmup 已完成。SGLang 故意让 HTTP 可连接，以便外部脚本能发 update 请求。
...
这段有两个重要失败模式：`dp_size` 不满足条件会直接失败；非 pause 状态下，update 要等 writer lock，避免模型状态和请求并发读写。
...
这一层只做请求类型到 handler 的路由。pause、flush、barrier 和 worker 调用都在 WeightUpdater。

## 第 10 步：WeightUpdater 负责 flush、metrics 和 barrier

IPC update 在 `_observe_weight_load("ipc")` 里执行。它先更新主 TP worker，再按需更新 draft worker；只要主 TP 成功，就 flush cache；最后做 TP CPU group barrier。
...
这里解释了为什么热更新 duration 是边沿触发，而不是普通 stats tick 写入。
...
| 验证点 | 方法 | 预期 |
|--------|------|------|
| 等待模式 | 加 `--checkpoint-engine-wait-weights-before-ready` 启动 | warmup 前等待 `initial_weights_loaded` |
| HTTP 可达 | 外部脚本轮询 `/ping` | `/ping` 成功只表示 HTTP listen |
| 初始权重 ready | 首次成功 POST `/update_weights_from_ipc` | HTTP endpoint 把 `initial_weights_loaded` 置为 `True` |
...
| cache flush | 使用默认 `flush_cache=True` 热更新 | 更新后 prefix cache 命中下降是预期现象 |
| metrics | 开启 metrics 后热更新 | `weight_load_duration_seconds{source="ipc"}` 更新 |
| post hook | 量化模型热更新 | 不应出现 `Post-hook processing failed` warning |

## 复盘

- CheckpointEngine 主线是控制面和适配层，不是 SGLang 内部实现 ParameterServer。
- `/ping` 可达、初始权重 ready、warmup 完成是三件事。
- IPC HTTP body 传 handles，真实 tensor 走外部 checkpoint-engine ZMQ 通道。
- Scheduler WeightUpdater 负责把权重替换变成 serving 安全边界：observe、flush、barrier。
