...
- [ ] 能沿 `ServerArgs.enable_metrics -> add_prometheus_middleware -> MultiProcessCollector` 复述 scrape lane。
...
- [ ] 能说明 `enable_metrics_for_all_schedulers` 会带来 rank label cardinality。
...
- [ ] `/metrics` 404 时，你会先查哪两个源码函数？
- [ ] `/metrics` 有 HTTP counter，但没有 `cache_hit_rate`，你会如何分层排查？
- [ ] 多 TP 部署里只有 TP0 有 Scheduler gauge，这是 bug 还是默认设计？
...
| 验证目标 | 操作 | 预期现象 |
|----------|------|----------|
| scrape 入口 | 启动时加 `--enable-metrics`，访问 `/metrics` | 返回 Prometheus 文本；关闭开关时不应有同样的挂载 |
...
| Tokenizer latency | 在 `TokenizerManager.collect_metrics` 打断点 | 首 token 写 TTFT，后续 chunk 写 ITL，finished 写 E2E |
...
4. label 和 rank 问题大多不是 Prometheus 查询问题，而是 collector 初始化和写入边界问题。
