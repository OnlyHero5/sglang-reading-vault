...
| Tokenizer 请求账 | TokenizerManager | 首 token、每次 streaming chunk、请求完成 | TTFT、ITL、E2E、prompt/generation tokens、cached tokens |
...
custom label 配置只影响 Tokenizer 请求账。允许的 label 会先加入 collector 的 label schema，后续请求完成时才能填入具体值。
...
这个设计避免 TP 副本重复写同一类 gauge。需要看每个 scheduler rank 时，才打开 `enable_metrics_for_all_schedulers`，同时 Grafana 要按 rank label 过滤。
...
TokenizerManager 初始化 request metrics collector 时先建立 label schema。允许的 custom labels 会先以空值加入，后续请求完成时再覆盖成具体值。
...
请求路径里，首 token 时 observe TTFT，后续 chunk observe ITL，finish 时 observe E2E 和 token counters。
...
这里的系统压力是 per-request latency 不能等 stats tick。TTFT/ITL/E2E 必须在请求输出路径按事件写入。
...
- label schema 在 collector 创建时决定；请求完成时只能填已存在的 label。
