...
| 症状 | 优先归类 | 第一入口 | 直接验证 |
|------|----------|----------|----------|
| `/metrics` 404 | scrape 入口账 | `add_prometheus_middleware` | 是否开启 `--enable-metrics` |
...
先不要查 `SchedulerStats`。`/metrics` 是 HTTP worker 挂载的 scrape 面，开关是 `server_args.enable_metrics`。
...
- `curl http://host:port/metrics`，确认不是 404 或 307。
...
- 生产默认不建议打开全量 rank，因为 cardinality 会随副本数增长。
...
它们写入时机不同。首 token 写 TTFT；后续有新增 completion tokens 的 chunk 写 ITL；请求 finished 才写 E2E 和 token counters。
...
- ITL 依赖 completion tokens 增量，空 chunk 不会写。
