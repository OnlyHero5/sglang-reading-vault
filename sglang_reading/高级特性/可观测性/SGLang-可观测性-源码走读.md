...
- Prometheus 发起一次 `GET /metrics`，HTTP worker 如何把 multiprocess registry 暴露出去。
- Scheduler 经过一次 stats tick，如何把 waiting queue、KV pool、cache hit、spec、LoRA、HiCache 写成 gauge。
- 一个生成请求输出首 token、后续 token、最终完成时，TokenizerManager 如何写 TTFT、ITL、E2E 和 token counters。
...
| 读者任务 | 先读 | 要抓住的判断 |
|----------|------|--------------|
| `/metrics` 为空或 404 | 第 1 到第 4 步 | `enable_metrics`、multiprocess 目录、HTTP mount 和 response middleware 是 scrape 面，不负责业务指标含义 |
| queue、KV、cache hit、LoRA 指标异常 | 第 5 到第 8 步 | Scheduler 只在指定 rank 或 all-scheduler 模式写状态账，decode 重统计按 interval 执行 |
| TTFT、ITL、E2E 不符合预期 | 第 9、第 10 步 | TokenizerManager 在首 token、后续 token、完成三个时刻写不同 histogram/counter |
...
flowchart TB
  ...
  B --> C["HTTP lifespan<br/>挂载 /metrics"]
  C --> D["Prometheus scrape<br/>MultiProcessCollector 聚合"]
...
如果这个目录错了，后面的 collector 即使创建成功，也可能写不到 Prometheus scrape 能聚合的位置。

## 第 3 步：HTTP worker 只负责挂载 scrape 入口
...
这里的系统压力是避免重复上报和控制 label 维度。排查 “为什么只有 TP0 有数据” 时，要回到这一步。
...
请求完成路径里会拿到 request 上的 custom labels，并覆盖默认空值。priority 也在这里写入 label。
...
## 第 10 步：首 token、后续 token、完成分别写不同指标

TokenizerManager 在首 token 观测 TTFT；后续输出观测 ITL；请求 finished 时观测 E2E 和 token counters。
...
ITL 使用每 token 平均 interval，并一次性增加多个 bucket 计数，避免逐 token observe 的开销。
...
这里的系统压力是请求热路径开销。SGLang 没有在每个 token 上做昂贵操作，而是按 chunk 聚合。
...
| 验证点 | 方法 | 预期 |
|--------|------|------|
| `/metrics` 是否挂载 | 启动时加或去掉 `--enable-metrics`，访问 `/metrics` | 开启后有 Prometheus 文本；关闭时没有该挂载 |
...
| TTFT/ITL/E2E 时机 | 在 `TokenizerManager.collect_metrics` 打断点 | 首 token 写 TTFT，后续 chunk 写 ITL，finish 写 E2E |
| RequestLogger 是否独立 | 关闭 metrics 但开启 `--log-requests` | 仍可写 request received/finished 日志 |
| custom labels | 配置 allowed labels 并发送对应 header | label schema 中有该 label，请求完成时填入具体值 |

## 复盘

- `/metrics` 是读取面，Scheduler 和 TokenizerManager 才是主要写入者。
- Scheduler 状态账是按窗口聚合的系统视角，不是单请求视角。
- Tokenizer 请求账按输出事件写 TTFT、ITL、E2E。
