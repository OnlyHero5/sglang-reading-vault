...
- Qwen3 decode KV cache 异常：看 fused mRoPE 路径和 `save_kv_cache`。
...
| 读者任务 | 先读 | 要抓住的判断 |
|----------|------|--------------|
| 模型加载时 architecture 不支持 | 1 到 3 | `get_model_architecture` 和 Registry 只做候选解析与类命中，真正实现来自 `EntryClass` |
...
| 排查 attention 输出或 KV cache 异常 | 9 到 14 | 模型层只准备 Q/K/V、QK-Norm、RoPE 和 save_kv_cache 边界，真正 attention 仍交给 `RadixAttention` |
...
Registry 的 `resolve_model_cls` 不检查模型结构。它只把字符串列表变成“已注册候选 + Transformers fallback”，然后按顺序返回第一个可用类。
...
## 3. EntryClass 是模型文件对 Registry 的最小契约

Registry 扫描模块时只认 `EntryClass`。Llama 文件一次暴露多个继承类，它们共享实现，但在 Registry 中是不同 architecture key。
...
这个片段建立了两个不变量：非 first PP rank 不应该消费原始 `input_ids` 生成 embedding；当前 rank 只执行 `[start_layer, end_layer)`。
...
如果当前 rank 不是最后一个 PP stage，输出不是 logits，而是 `PPProxyTensors`。这也是 PP 中间 rank 没有 `lm_head` 行为的根因。
...
Qwen3 attention 和 Llama 的第一个差异是并行组：它使用 `attn_tp_rank/attn_tp_size` 构造 QKV 和 output projection，并创建 `q_norm/k_norm`。
...
如果排查 Qwen3 head shape 或 DP-Attention 相关问题，优先看这里，而不是 Llama 的 `tp_size` 分支。
...
## 12. fused mRoPE 路径会提前写 KV cache

Qwen3 的 aiter fused mRoPE 路径在 decode 中把 QK-Norm、mRoPE 和 KV cache 写入合在一起。函数返回 `(q, None, None)`，调用方必须让 attention 不再重复写 KV。
...
Qwen3 只在 fused 能力存在、decode 模式、非 RL on-policy 时走 fused 路径。走 fused 后把 `save_kv_cache` 改成 False。
...
这就是 Qwen3 相比 Llama 更复杂的地方：不是 decoder 顺序变了，而是 attention/MLP 边界的通信和 layout 被抽象出来。
...
## 16. Qwen3 load_weights 还要补 `model.` 前缀和 tied embedding

Qwen3 的 `load_weights` 和 Llama 类似，但对缺少 `model.` 前缀的 checkpoint name 做兼容；tied embedding 场景还可能把 `model.embed_tokens.weight` 写到 `lm_head.weight`。
...
后续仍然是 stage 过滤、skip、scale remap、stacked mapping 和 fallback loader。
...
| fused mRoPE 后 KV 异常 | `use_aiter_fused`、`save_kv_cache` | fused decode 已写 cache，后续应传 `save_kv_cache=False` |
