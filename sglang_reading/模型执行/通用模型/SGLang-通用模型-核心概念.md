...
Registry 的表项来自模型文件的 `EntryClass`。导入 `sglang.srt.models.registry` 时，内置模型包会被扫描；如果设置了外部模型包，还会用 `overwrite=True` 注册外部实现。
...
这里的 key 是类名，例如 `Qwen3ForCausalLM`。所以接新模型时，最小契约不是“文件名叫 qwen3”，而是 HF `architectures` 里的字符串能命中 `EntryClass.__name__`。
...
这解释了 native 与 Transformers fallback 的边界：如果本地支持或用户没有强制 Transformers，就优先走 native 模型类；否则把 architecture 改写成通用 wrapper 再 resolve。
...
因此，PP 模型不是每个 rank 都持有完整层。它保持完整 `ModuleList` 下标空间，但 stage 外位置是占位层。
...
Qwen3 和 Llama 的骨架相近，但 Qwen3 attention 使用 attention TP 组，并在 Q/K 上做 RMSNorm。
...
因此，ModelLoader 只把 `(name, tensor)` 送进来；模型类决定名字如何翻译；参数对象决定如何按 TP rank 和 shard 写入。
