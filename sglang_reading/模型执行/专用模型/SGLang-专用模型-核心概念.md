...
- shared expert fusion、DeepEP fusion、独立 shared expert 三种形态如何影响 expert 数。
...
所以 architecture 命中是类账问题：HF config 里的 `architectures` 决定 Registry 命中哪个类名；后续是否启用 DSA、shared expert fusion、CP，则看 config 字段和 server args。
...
`DeepseekV2AttentionMLA` 本身只做两件关键事：构造 latent projection 和两个 `RadixAttention` 实例，然后在 forward 时按方法走不同 prepare/core：
...
这里的心理模型是：MLA 和 MHA 都还是 attention adapter，只是 KV cache 的物理形态和 head/dim 元数据不同。prefix tree、调度 batch、采样输出都不在这个类里决定。
...
`DeepseekV2MoE` 的第一层判断是 expert 数和 shared expert 形态。DeepEP fusion 会把 shared expert 表现成额外 slot，普通 fusion 则把 routed expert 数加上 fused shared expert 数：
...
因此看到 `num_experts` 多出来，不要马上判断 checkpoint 错。先看是否是 shared expert fusion 或 DeepEP fusion。
...
这解释了一个常见误区：DeepSeek 层内顺序看起来仍是 norm、attention、norm、MLP，但真正影响 layout 的判断已经交给 communicator 和 `LayerScatterModes`。
...
权重账的核心不是“找到文件”，而是把 checkpoint 的名字和运行时参数形态对齐：PP stage 外权重要跳过，shared expert 可能 remap 到 `mlp.experts.<n_routed_experts>`，`q_a_proj` 和 `kv_a_proj_with_mqa` 可能合成 `fused_qkv_a_proj_with_mqa`，DSA indexer 可能把多个权重灌到 fused 参数里。
