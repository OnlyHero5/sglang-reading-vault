...
- [ ] 能解释 `ScheduleBatch` 和 `ForwardBatch` 的边界，并说出 `input_ids`、`req_pool_indices`、`seq_lens`、`out_cache_loc` 四个字段的用途。
...
| 执行快照构造 | `ForwardBatch.init_new` |
| graph/eager 选路 | `ModelRunner._forward_raw` |
| eager 真正调用模型 | `EagerRunner._execute_decode` / `_execute_extend` |
...
- [ ] 启动一次带 graph 的服务，记录 decode graph capture 日志；再用关闭 graph 的配置对照吞吐。
