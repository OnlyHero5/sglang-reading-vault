...
**Explain：** 就绪判定标志是 HTTP health endpoint 返回 OK 且 Scheduler 完成 warmup forward。
...
**Comment：** 若 load_model 失败，子进程 exit，HTTP 不会进入 listen。
...
**Explain：** 连续批处理允许 Waiting 中多个请求与 Decoding 中请求在同一 GPU batch 共存。
...
**Comment：** 业务上「一次生成」= Scheduler 侧 Req 从 Waiting 到 Finished 的完整生命周期。
...
**Explain：** 业务 KPI 是 `cached_tokens` 字段——HTTP 响应 meta_info 中可见。
...
**Comment：** LoRA adapter 不同则 extra_key 不同，**不会错误共享** KV——这是多租户场景的关键隔离。
...
**Explain：** 业务收益 = decode throughput ↑；风险 = draft 质量低则 accept 少，overhead 反而增加。
...
**Comment：** 启用需 `--speculative-algorithm eagle` 等 CLI 参数，见 ServerArgs。
...
**Explain：** 业务上需独立扩缩 Prefill vs Decode 算力——prompt 长时扩 Prefill，QPS 高时扩 Decode。
...
**Comment：** Mooncake/NIXL 等 backend 对网络拓扑有要求，见 batch 22 专题。
...
**Explain：** 业务约束是 `max_loras_per_batch`——单 batch 最多 N 个不同 adapter。
...
**Explain：** 预处理可在 CPU（主进程）或 GPU（IPC transport），取决于 Processor 实现。
...
**Comment：** 投机+grammar 需 accept/rollback 协同，是域 4 与域 8 的交叉点。
