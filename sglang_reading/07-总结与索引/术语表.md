...
> 完全没接触过 LLM serving？先看 [[00-零基础先修]]。下表用一句话 + 生活类比快速建立直觉；细节点「深读」链到对应文档。

| 术语 | 一句话定义 | 生活类比 | 深读 |
|------|-----------|----------|------|
| **LLM Serving** | 把大模型部署成在线服务，多用户并发、流式返回答案 | 智能餐厅：接单、做菜、上菜一条龙 | [[00-零基础先修|00-零基础先修]] §1、[[01-项目总览|01-项目总览]] |
| **Prefill** | 一次性处理整段 prompt，写入 KV Cache | 备菜：把订单上所有食材先切好腌好 | [[00-零基础先修|00-零基础先修]] §2、[[13-Models-通用-03-数据流与交互|13-Models-通用/03-数据流]] |
| **Decode** | 每步只生成 1 个新 token，循环直到结束 | 逐道上菜：炒好一道端一道，直到结账 | [[00-零基础先修|00-零基础先修]] §2、[[07-Scheduler-00-MOC]] |
| **TTFT** | 从请求进入到收到第一个 token 的耗时 | 点单后等第一道菜的时间 | [[00-零基础先修|00-零基础先修]] §3、[[09-生产排障速查|09-生产排障速查]] §2 |
| **TPOT / ITL** | 首 token 之后，每两个 token 之间的平均间隔 | 每两道菜之间的等待时间 | [[00-零基础先修|00-零基础先修]] §3、[[31-Observability-00-MOC]] |
| **Token** | 模型处理文本的最小单位（可能是字、词或子词） | 菜单上的「份」——整句被切成可计价的小份 | [[06-TokenizerManager-00-MOC]] |
| **KV Cache** | 存已算过的 Key/Value 向量，避免 decode 时重复计算 | 服务员记事本：翻过的页不用重抄 | [[00-零基础先修|00-零基础先修]] §4、[[16-KV-Cache-00-MOC]] |
| **Continuous Batching** | 动态合并 prefill 与 decode 请求，提高 GPU 利用率 | 拼桌：有人吃完不撤桌，新客随时加入 | [[00-零基础先修|00-零基础先修]] §5、[[07-Scheduler-00-MOC]] |
| **TokenizerManager** | 主进程：HTTP 接入、tokenize、流式响应组装 | 餐厅前台：接单、记备注、收银 | [[00-零基础先修|00-零基础先修]] §6、[[06-TokenizerManager-00-MOC]] |
| **Scheduler** | 子进程：组 batch、驱动 GPU forward、管理队列 | 调度员 + 后厨入口：决定这轮做哪几桌 | [[00-零基础先修|00-零基础先修]] §6、[[07-Scheduler-00-MOC]] |
| **DetokenizerManager** | 子进程：token id 转 UTF-8 文本 | 传菜员：把厨房编号翻译成菜名 | [[00-零基础先修|00-零基础先修]] §6、[[10-Detokenizer-00-MOC]] |
| **ZMQ IPC** | 三进程间传递结构化消息的高性能通道 | 前台 ↔ 后厨的内部对讲机 | [[03-HTTP-Server-03-数据流与交互|03-HTTP-Server]]、[[全链路请求追踪|全链路请求追踪]] |
| **ForwardMode** | 区分 prefill（EXTEND）与 decode（DECODE）等 forward 类型 | 备菜模式 vs 单道上菜模式 | [[00-零基础先修|00-零基础先修]] §2、[[09-ScheduleBatch-IO-00-MOC]] |
| **Prefix Cache** | 复用相同 prompt 前缀的 KV，跳过重复 prefill | 多家分店共用同一份今日特价菜单 | [[00-零基础先修|00-零基础先修]] §7、[[15-RadixAttention-00-MOC]] |
| **RadixAttention** | 用 radix tree 组织前缀 KV 的 SGLang 核心特性 | 按菜单目录树共享相同章节 | [[00-零基础先修|00-零基础先修]] §7、[[15-RadixAttention-00-MOC]] |
| **RadixCache** | 前缀缓存的具体实现类，含 match/insert/evict | 菜单档案柜：查、存、腾位置 | [[15-RadixAttention-00-MOC]]、[[16-KV-Cache-00-MOC]] |
| **Streaming / SSE** | 生成一个 token 就推一条，不等全文完成 | 逐道上菜，不用等满汉全席摆齐 | [[03-HTTP-Server-00-MOC]]、[[全链路请求追踪|全链路请求追踪]] |
| **ModelRunner** | 加载权重、管理 KV pool、执行模型 forward | 后厨灶台：真正炒菜的地方 | [[11-ModelRunner-00-MOC]]、[[03-关键概念|03-关键概念]] |
| **PD Disaggregation** | Prefill 与 Decode 拆到不同机器/池 | 备菜厨房与上菜厨房分开，各自扩缩 | [[22-Disaggregation-00-MOC]]、[[07-用户故事与场景|07-用户故事]] 故事 C |
| **Speculative Decoding** | 用小模型草稿 + 大模型验证，加速 decode | 学徒先猜下一道，主厨快速验对错 | [[21-Speculative-00-MOC]]、[[07-用户故事与场景|07-用户故事]] 故事 B |
...
**Explain：** 与 `BatchTokenIDOutput` 成对，是 ZMQ 回程消息类型。
