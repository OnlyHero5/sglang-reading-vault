...
> 读完本篇，再进入 [[01-项目总览|01-项目总览]] 与 [[04-导读路径|04-导读路径]] 
...
本篇用**餐厅类比**建立直觉，再用极简 mermaid 和少量源码锚点，帮你读懂后续文档里反复出现的术语。全文采用 **ETC 格式**（Explain → Code → Comment），与全库一致。不必先学 PyTorch 或 CUDA；只需知道「模型在 GPU 上算、请求走网络进来、回答一个字一个字出来」。
...
- 专题：[[13-Models-通用-03-数据流与交互|13-Models-通用/03-数据流]]、[[07-Scheduler-00-MOC]]
...
- SLA 常写「P99 TTFT < 500ms」「P99 TPOT < 50ms」——两者可能冲突。排障见 [[09-生产排障速查|09-生产排障速查]] §2、§3。
...
- 专题深读 [[16-KV-Cache-00-MOC|04-内存与Attention/16-KV-Cache/]]
...
- 专题深读 [[07-Scheduler-00-MOC|02-请求调度/07-Scheduler/]]、[[08-SchedulePolicy-00-MOC|02-请求调度/08-SchedulePolicy/]]
...
- 理解「Detokenizer 是 Scheduler 回程第一站」后，全链路七 hop 就不会迷路。见 [[全链路请求追踪]]。
- 专题深读 [[03-HTTP-Server-00-MOC|01-启动与入口/03-HTTP-Server/]]、[[06-TokenizerManager-00-MOC|02-请求调度/06-TokenizerManager/]]
...
- 专题深读 [[15-RadixAttention-00-MOC|04-内存与Attention/15-RadixAttention/]]、[[07-用户故事与场景]] 故事 A
...
00-方法论 · ETC 格式与阅读地图
 ↓
01-项目总览 · monorepo 与启动链
 ↓
全链路请求追踪 · HTTP 七 hop 串起来
 ↓
04-导读路径 Step 1–8 · 15 步 tour 的前半段
 ↓
...
| 阶段 | 文档 | 用时 |
|------|------|------|
| 0–4 | 本篇 → [[00-方法论-00-MOC]] → [[01-项目总览|01-项目总览]] → [[全链路请求追踪|全链路请求追踪]] → [[04-导读路径|04-导读路径]] Step 1–8 | 约 4 小时 |
| 5 | [[术语表|术语表]] / [[05-文件地图]]。
...
→ 无 LLM serving 经验：**必读** [[01-项目总览]] 
→ 想立刻看请求怎么跑：[[全链路请求追踪]] 
→ 按步导读：[[04-导读路径]] **Step 0** 已指向本篇
