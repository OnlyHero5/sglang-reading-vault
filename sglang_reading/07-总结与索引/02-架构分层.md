...
**Explain：** 读源码前先看本层，建立 monorepo 边界感。
...
**Explain：** `sglang serve` 的默认路径：`cli/main` → `cli/serve` → `launch_server` → `http_server.launch_server`。
...
**Explain：** `OpenAIServingBase` 模板方法统一校验、转换、流式响应；Ollama 为平行适配器。
...
**Comment：** HTTP 主进程与子进程 Scheduler/Detokenizer 通过 **ZMQ + 不同 port** 通信（见 `io_struct.py` 消息定义）。
...
**Explain：** 本层是 SGLang 吞吐量的核心——`event_loop_overlap` 让 CPU 调度与 GPU 前向流水线并行。
...
**Explain：** RadixAttention 是 SGLang 相对 vLLM 的差异化能力——跨请求共享相同 prompt 前缀的 KV。
...
| 组件 | 目录 | 模块 |
|------|------|------|
| sgl-kernel | `sgl-kernel/python/sgl_kernel/` | 26 |
| model-gateway | `sgl-model-gateway/src/` | 27 |
| Frontend lang | `python/sglang/lang/` | 28 |
| multimodal_gen | `python/sglang/multimodal_gen/` | 29 |
| VLM / LoRA | `srt/multimodal/`, `srt/lora/` | 24–25 |
...
**Explain：** 易错点——`from sglang import Engine` 实际得到的是 **Runtime Engine**（LazyImport 覆盖 Frontend Engine）。
