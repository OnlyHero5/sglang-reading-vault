...
| 参数对象 | `filtered_argv` | `ServerArgs` 实例 | `srt/server_args.py`（[[SGLang-启动链路-核心概念|启动链路 详述]]） |
| 服务进程 | `ServerArgs` | 监听 HTTP/gRPC 端口 | `srt/entrypoints/*`（HTTP Server–gRPC/Proto） |

**读法：** `ServerArgs` 是全局配置单例的 dataclass——CLI 每个 flag 对应一个字段，`prepare_server_args` 解析 argv 后 `__post_init__` 做交叉校验。本模块只需知道「argv 在此变成对象」；字段含义见 启动链路。
...
**要点：** `dispatch_argv` 是剥掉 `--model-type` 后的 argv，与 `python -m sglang.launch_server` 的 `sys.argv[1:]` 等价。
...
**要点：** `get_is_diffusion_model` 读模型 config 判断架构类型。
...
**要点：** Frontend 不嵌入 torch 推理，而是把 `gen()` 等调用翻译成对 Runtime HTTP API 的请求。
...
```powershell
rg -n 'def main|add_parser\("serve"|def serve|ServerArgs|def run_server|launch_server|RuntimeEndpoint|kill_process_tree' sglang/python/sglang/cli/main.py sglang/python/sglang/cli/serve.py sglang/python/sglang/launch_server.py sglang/python/sglang/srt/server_args.py sglang/python/sglang/__init__.py
```

预期输出应同时覆盖 `cli/main.py`、`cli/serve.py`、`launch_server.py`、`server_args.py` 和 `__init__.py`。如果其中某个入口消失，说明本页的“命令到 HTTP 服务”主线需要重新核对。
