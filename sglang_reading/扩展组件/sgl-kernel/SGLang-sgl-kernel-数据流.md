...
**读法：** sgl-kernel 位于 srt **计算层最底端**，不参与请求调度或 HTTP 协议。数据流是单向的：srt 上层构造好 GPU tensor → Python 薄封装校验 → CUDA kernel 写回 output tensor。
...
**读法：** 绝大多数算子遵循「预分配 output / in-place buffer」模式，避免 Python GC 与 CUDA sync 开销。
...
**读法：** 一次 MoE layer forward 中，sgl-kernel 参与 4 个阶段：路由 → 对齐 → grouped GEMM → 聚合。
...
**读法：** 任何 `import sgl_kernel` 都会触发完整加载链，与具体算子调用无关。
...
**读法：** prefill worker 完成一层 forward 后，通过 `transfer_kv_per_layer` 将 K/V 按 slot index 拷贝到 decode worker 的 paged buffer。
...
读输出时先看 `load_utils.py` 的 compute capability 与 CUDA library preload，再看 `attention.py` / `moe.py` 的 Python wrapper 是否只做参数整理并调用 `torch.ops.sgl_kernel`。KV 搬运问题则直接定位 `kvcacheio.py` 的 per-layer、all-layer、MLA 三组入口。
