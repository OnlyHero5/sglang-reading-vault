...
| 读者任务 | 先读 | 要抓住的判断 |
|----------|------|--------------|
| 首次建立 LoRA serving 主线 | 贯穿场景、第 1 到 2 步 | 控制面先把用户名称稳定映射为内部 `lora_id`，未启用 LoRA 的请求会在这里失败 |
...
| 排查 kernel metadata 错位 | 第 8 到 9 步 | backend 看到的是 `weight_indices`、rank、scaling、segment，不再是用户传入的路径名 |
| 改 LoRA target module 或 backend | 第 9 到 10 步、运行验证 | 包装层负责 `base_output + delta`，backend 负责按 token/segment 执行 LoRA A/B 路径 |
...
`_resolve_lora_path` 还会尝试把曾经加载过、后来被 unregister 的 adapter 重新 load 回来。最后一步才是 `LoRARegistry.acquire`，它把 `lora_path` 换成 `lora_id` 并开始引用计数。
...
`ForwardBatch` 是模型执行前的快照。它从 `batch.reqs` 中收集 `lora_ids`，再在 LoRA 打开时调用 `fetch_new_loras` 和 `prepare_lora_batch`。
...
`validate_lora_batch` 先看 batch 内唯一 `lora_id` 数量是否超过 `max_loras_per_batch`，再把 pinned adapter 从可换出槽位里扣掉。这里解释了为什么 pinned adapter 太多会让普通请求或 base model 饥饿。
...
这一步把“adapter 身份”变成“kernel 可消费的索引”。后面 layers 和 Triton kernel 不再关心 `lora_name` 或 path。
...
最小验证不需要改源码。启动服务时打开 LoRA，并预加载一个 adapter：
...
- LoRA serving 的主线不是文件顺序，而是 adapter 身份逐步变成 GPU slot 的过程。
- `lora_id` 同时服务三件事：请求身份、cache namespace、执行面 uid。
- Scheduler 的 LoRA 逻辑是准入控制，不是权重加载本体。
- `LoRAMemoryPool` 的安全性来自三点：不换出当前 batch、不换出 pinned adapter、缺失权重清零。
- backend 和 layer 只消费 batch metadata，因此 adapter 名称错误通常应该先查 TokenizerManager 和 registry。

下一篇 [[SGLang-LoRA-数据流]] 会把这条主线拆成控制流、cache 流、slot 流和动态加载流。
