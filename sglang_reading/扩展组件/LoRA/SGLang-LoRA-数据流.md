...
这篇把 [[SGLang-LoRA-源码走读]] 的主线拆成四条数据流：
...
这解释了一个重要排障点：如果 backend load 失败，用户不会在 registry 里看到一个半加载 adapter。
...
这条流的安全性来自两阶段顺序：先让新请求进不来，再等旧请求走完。
...
所以 overlap loading 是延迟隐藏策略，不改变容量不变量。

## 流 8：drainer 是公平性闸门，不是卸载器

当 batch 内 adapter 种类占满，某些 adapter 长时间等不到机会，drainer 会把正在运行的 adapter 标记为 draining，限制它继续接收新请求，让等待过久的 adapter 有机会进入。
...
| unload 不杀 in-flight 请求 | 对同一 adapter 发长生成请求，同时调用 unload | unload 卡在 `wait_for_unload`，长请求结束后后端卸载 |
...
| drainer 只影响准入 | 设置很小 `lora_drain_wait_threshold` | 某些 adapter 的新请求暂时不被 schedule，但 registry 和 memory pool 不被删除 |
