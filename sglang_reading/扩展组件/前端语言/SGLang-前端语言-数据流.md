...
SGL 前端最容易误读为“字符串拼接语法糖”。实际上，Python 表达式会先变成 IR，再由解释器维护 program state、fork/join 与 stream，最后交给 backend 发出真实请求。本文追踪这条语义降级过程，说明问题是在 DSL、解释器还是推理后端。
...
**读法：** 多样本共享 instruction prefix 时，trace 提取 prefix → 一次 cache → 并行 run。
...
**读法：** fork 复制 executor 状态到 N 个分支，各分支独立 gen，join 可选 concate KV 或文本。
