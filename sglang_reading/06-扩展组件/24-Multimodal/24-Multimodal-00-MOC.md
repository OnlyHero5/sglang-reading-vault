...
**Explain：** 多模态推理位于 TokenizerManager 与 Scheduler 之间：Processor 将 OpenAI messages 中的 image/video/audio 转为 `input_ids` placeholder 与 `MultimodalDataItem` feature tensor。ViT/Audio Tower 在 Scheduler prefill 阶段 forward，embedding 与 text token 拼接后进入 LLM 主干。本模块与multimodal_gen-multimodal_gen（扩散生成）不同，聚焦 VLM 对话推理路径。
...
| 文件 | 内容 |
|------|------|
| [[24-Multimodal-01-核心概念]] | Modality、Processor 注册、Special Tokens |
| [[24-Multimodal-02-源码走读]] | base_processor、qwen_vl、multimodal_processor |
| [[24-Multimodal-03-数据流与交互]] | TokenizerManager → Scheduler → ViT |
| [[24-Multimodal-04-关键问题]] | 新模型接入、IPC、PD+Encoder |
| [[24-Multimodal-05-checkpoint]] | 验收清单 |
...
**Explain：** 启动时 `import_processors` 扫描 processors 包，将 `BaseMultimodalProcessor` 子类注册到 `PROCESSOR_MAPPING`；运行时按 `hf_config.architectures` 选取。
