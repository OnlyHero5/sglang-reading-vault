...
**Explain：** `MultimodalDataItem` 挂在 `Req` 上进入 Scheduler；prefill 时 vision tower 消费 item，产出 embedding 写入 hidden states。
...
**Comment：** 具体字段因模态而异；读 [[09-ScheduleBatch-IO-00-MOC|ScheduleBatch-IO]]可对照完整 dataclass。
...
**Explain：** PD + VLM 时 encoder 可分离部署，embedding 经 encode_grpc 送达 prefill。
...
**Comment：** 与PD 分离 PD 文档交叉引用。

---

## 4. pad_input_ids 与 placeholder 展开

**Explain：** 单个 `<image>` placeholder 需展开为 N 个 image pad token（N 由分辨率与 patch 决定）；Processor 负责计算 N 并替换文本。
...
**Comment：** `multimodal_processor.py` 顶部 TODO 计划将 pad_input_ids 移入本模块统一维护。

---

## 5. IPC 跨进程路径

**Explain：** TokenizerManager 与 GPU Scheduler 分离时，feature tensor 序列化为 CUDA IPC handle，Scheduler 端 reopen。
...
**Comment：** 关闭 IPC 时走传统 pickle + CPU tensor，延迟更高但调试简单。

---

## 6. customized_mm_processor_utils

**Explain：** 用户可注入自定义预处理 hook，无需 fork 整个 Processor 类。
