...
`out` 用来计算 softmax backward 的行标量 `D=sum(dO*O)`。只有 `Q/K/V/LSE` 不够，因为 `D` 依赖 forward 输出和上游梯度。
...
排障抓手：如果 `out` shape 被裁剪、padding 不一致或传错 buffer，`D` 会错，后续 `dS` 全部建立在错误行标量上。
...
验证抓手：跑同一个输入，分别设置 `deterministic=False/True`，观察 peak memory 与耗时。正确预期是 deterministic 更慢或占更多临时空间，但梯度语义不变。
