...
| 排查梯度问题 | 从 `softmax_lse`、dropout `rng_state`、deterministic、GQA sum、varlen layout 定位错误边界 |
...
| `csrc/flash_attn/src/flash_bwd_preprocess_kernel.h` | 计算 `D = sum(dO * O)`，并清理非 deterministic 的 `dQaccum` |
...
核心判断是：Backward 不是把 forward 简单反过来跑，而是用 forward 留下的压缩状态在 tile 内重建同一个 softmax 结果，再把 softmax 反向公式映射成几组 tiled GEMM。
