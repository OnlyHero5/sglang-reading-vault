...
```mermaid
flowchart LR
    A["FA1<br/>HBM traffic"]
    B["FA2<br/>并行分工 + API 工程化"]
    C["FA2.1-2.7<br/>decode / paged / softcap / compile"]
    D["FA3<br/>Hopper 硬件映射"]
    E["FA4<br/>CuTeDSL / JIT 维护成本"]
    A --> B --> C --> D --> E
```
...
| FA2 | 主包重写、work partitioning、varlen API、C++/CUDA specialization | 把算法原点变成可被 PyTorch/训练框架调用的稳定 extension | [[FlashAttention-Python-API]]、[[FlashAttention-FA2-Forward]] |
...
| FA4 | CuTeDSL、Hopper/Blackwell、`flash_attn.cute` API、JIT/cache | 用 DSL/JIT 管理更多架构、shape、dtype 和 mask 组合 | [[FlashAttention-FA4-CuTeDSL演进]] |
...
| 为什么 decode 不只是普通 forward 小 batch | FA2.2+ | 小 `seqlen_q` 的瓶颈转向 KV cache 读取 |
| 为什么 paged KV、softcap、ALiBi 进入 attention backend | FA2.3-2.7 | serving 和模型结构需求进入主包 |
| 为什么 H100 有单独路径 | FA3 | Hopper 的 TMA/GMMA/FP8 需要不同组织 |
| 为什么 FA4 要 JIT/cache | FA4 | 架构、shape、dtype、mask 组合继续膨胀，静态模板维护成本变高 |
...
| 2.2 | 小 query decode 优化；`flash_attn_with_kvcache` | 读 [[FlashAttention-KV-Cache-源码走读]] |
...
| FA2 | Python API 到 extension 到 template specialization | 不能直接解释 Hopper TMA/GMMA 和 CuTeDSL JIT |
| FA2.2+ | serving decode 是 KV cache IO 问题 | 不能把所有推理 backend 都等同于 `flash_attn_with_kvcache` |
| FA3 | Hopper 专门路径 | 不能假设它已经替代默认 FA2 主包 |
| FA4 | JIT/cache 管理复杂组合 | 不能把 JIT 首次编译问题误判为 attention 算法问题 |

## 运行验证

| 验证目标 | 操作 | 预期 |
|----------|------|------|
| 判断当前是否走 FA2 主包 | Python 中检查 `import flash_attn; flash_attn.__version__` | 当前基线为 `2.8.4` |
| 判断公开 API 是否存在 | `from flash_attn import flash_attn_func, flash_attn_with_kvcache` | dense 和 KV cache API 均可 import |
| 判断 FA3 是否可用 | 按 README 进入 `hopper` 安装并 import `flash_attn_3` | 需要 H100/H800 和 CUDA 12.3+ |
| 判断 FA4 是否可用 | 安装 `flash-attn-4` 后 import `flash_attn.cute` | 关注 CUDA 版本、GPU 架构和首次 JIT 编译 |
| 判断 decode 瓶颈 | 对小 `seqlen_q` decode 做 profile | 重点应落在 KV cache load 和 SplitKV combine |

## 复盘

四代增量的主线可以压成一句话：FA1 建立 IO-aware exact attention，FA2 把它工程化成主包 extension，FA2 中后期把 serving 和模型变体纳入主路径，FA3 面向 Hopper 重写硬件映射，FA4 用 CuTeDSL/JIT 管理继续膨胀的特性组合。
