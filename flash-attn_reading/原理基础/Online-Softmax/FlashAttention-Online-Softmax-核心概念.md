...
FlashAttention 的 IO 优化依赖一个前提：不能把完整 `S = QK^T` 和 `P = softmax(S)` 存下来，但输出必须等价于全量 softmax。Online softmax 就是这个前提的数学和源码共同边界。
...
| 分子账 | `acc_o` | 在当前 `row_max` 标尺下的未归一化 `P @ V` 累积 |
...
- `acc_s`：当前 K block 的 score tile，进入函数后会被原地改成概率 tile。
...
- 生成 `LSE = row_max * softmax_scale + log(row_sum)`，给 backward 重算 probability。
...
`softmax_lse` 的大小是每个 query 行一个标量；完整 `P` 是每个 query-key 对一个值。这就是训练时“不保存完整 attention matrix”仍能 backward 的状态压缩。
...
4. LSE 把最终最大值和分母合成一个稳定标量，是 forward/backward 的协议字段。
